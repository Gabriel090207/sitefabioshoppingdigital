...
const lojaId = `${nomeBase}-${Date.now()}`;

    
    const perfilRef = ref(
  storage,
  `lojas/${lojaId}/perfil-${fotoPerfil.name}`
);

const bannerRef = ref(
  storage,
  `lojas/${lojaId}/banner-${fotoBanner.name}`
);

const capaRef = ref(
  storage,
  `lojas/${lojaId}/capa-${fotoCapa.name}`
);
    ...
    const perfilURL = await getDownloadURL(perfilRef);
